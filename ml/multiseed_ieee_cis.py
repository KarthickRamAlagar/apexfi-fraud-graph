"""Multi-seed validation for IEEE-CIS's winning pipeline (LightGBM +
stacking). Scoped deliberately: only LightGBM's own internal training
randomness varies across seeds (bagging/feature subsampling) — the
train/val/test split stays FIXED (re-randomizing it would also shift
frequency-encoding statistics, a confound we don't want), and the GNN
is NOT retrained 3 times (its probabilities, from the single validated
checkpoint, are reused across all 3 seed runs) — this keeps the run
fast and low-risk given how fragile GNN training has been on this
hardware, while still giving real seed-to-seed variance on the model
...
def main():
    ...
    gnn_val_probs = get_gnn_probs(gnn, data_for_gnn, data.val_mask, shared_edges)
    gnn_test_probs = get_gnn_probs(gnn, data_for_gnn, data.test_mask, shared_edges)
    print("  done")
    ...
    with open("streamlit_app/data/multiseed_ieee_cis.json", "w") as f:
        json.dump(
            {"seeds": SEEDS, "lightgbm": lgbm_summary, "stacked": stacked_summary,
             "note": "GNN reused a single validated checkpoint across all seeds (not retrained per-seed), given GNN training's memory fragility on available hardware. LightGBM's own training randomness varied per seed; train/val/test split held fixed."},
            f, indent=2,
        )
    print("\nSaved to streamlit_app/data/multiseed_ieee_cis.json")
