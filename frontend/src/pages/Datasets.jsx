...
export default function Datasets() {
  ...
  return (
    <>
      <motion.div
        className="space-y-5 py-8 px-6 sm:px-8"
        animate={{ marginRight: inspectorOpen ? 320 : 0 }}
        transition={{ duration: 0.22, ease: 'easeInOut' }}
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-muted-foreground">ApexFi / Datasets</p>
            <h1 className="mt-1 font-display text-2xl font-semibold">Datasets</h1>
          </div>
          <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground sm:w-72">
            <Search size={14} />
            <input
              value={query}
          ...
          {filtered.map((ds, i) => (
            ...
            >
              <div className="flex-1">
                <h3 className="font-display text-base font-semibold">{ds.name}</h3>
