...
def main():
    ...
    chunks = []
    chunk_size = 50_000
    for i, chunk in enumerate(pd.read_sql("SELECT * FROM gold.ieee_cis_features", engine, chunksize=chunk_size)):
        chunks.append(chunk)
    ...
    x = torch.tensor(numeric_df.values, dtype=torch.float)
    y = torch.tensor(labels, dtype=torch.long)

    print("\nLoading edges from gold.ieee_cis_transaction_edges...")
    edges_df = pd.read_sql("SELECT src_transactionid, dst_transactionid FROM gold.ieee_cis_transaction_edges", engine)
    ...
    print("\nBuilding stratified train/val/test split (70/15/15)...")
