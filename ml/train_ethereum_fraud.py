...
def main():
    ...
    # Saved so the backend can report real, live numbers on Dashboard —
    # matching the pattern used for IEEE-CIS/DGraph-Fin's multiseed JSONs,
    # though this is a SINGLE run, not multi-seed validated (honestly
    # labeled as such in the saved file, and in how the backend reads it).
    metrics_payload = {
        "single_run": True,
        "seeds_validated": 1,
        "metrics": metrics,
        ...
        "fraud_rate_pct": round(float(df["FLAG"].mean()) * 100, 2),
    }
    with open("streamlit_app/data/model_metrics_ethereum.json", "w") as f:
        import json
        json.dump(metrics_payload, f, indent=2)
    print("Saved: streamlit_app/data/model_metrics_ethereum.json")
